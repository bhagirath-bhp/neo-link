...
import { listImageURLs, uploadImageAndSaveURL } from "@/supabase/supabase";
...
const ImageCarousel = (props) => {
  ...
  useEffect(()=>{
    const fetchImages = async () => {
        const response = await listImageURLs(props.userId);
...
//   ];
  ...
  const imageComponentSet = (Array.isArray(imageURLs)) && imageURLs?.map((img) => (
    <img
      src={img}
      key={Math.random()}
      alt="image 1"
        ...
        <DialogFooter>
          <Dialog open={isUploadOpen} size="xs">
            <form action="" onSubmit={handleSubmit}>
                ...
                <Button color="red" variant="outlined" onClick={handleUploadOpen}>Cancel</Button>
                </DialogFooter>
            </form>
          </Dialog>
          <div className="flex gap-[1rem]">
            <Button onClick={handleUploadOpen}>Add Images</Button>
            <Button onClick={handleOpen} color="red" variant="outlined">Cancel</Button>
          </div>
        </DialogFooter>
      </Dialog>
    </div>
